...
- `contentList[].thumbnail` にページ全体を写した画像(PNG / JPEG、幅 480px 以上、縦横比はページと同じ)の URL を渡すと、大きく縮めた表示(倍率 0.5 未満)で紙面が iframe ではなくその画像になり、縮小が軽くなる(実測 p95 33ms → 17ms)。内容が変わったら URL も変える(`&v=…`)。渡さないページ・画像が読めなかったページは今までどおり iframe(0.4.0)
