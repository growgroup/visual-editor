...
- **コメントツールにした時点で右のコメントパネルが開く**(0.4.1)。範囲を描き終えるまで開かないと、書き込む先が見えないまま範囲を引くことになる。入口はどれも `activeTool` を変えるだけなので、`FrontendVisualEditor` の `useEffect([activeTool])` 1 か所で拾う。ツールを抜けても閉じない(閉じるのは利用者の操作)
