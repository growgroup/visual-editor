# 改行・Enter・サイズ欄の修正と、最小・最大の幅と高さ(0.7.0)

不具合 3 件(`<br>` がレイヤーに出る / Enter で段落が分かれる / サイズ欄の数値が切れる)と、
足りなかった機能 1 件(min / max-width・height の編集)。

## 1. `<br>` がレイヤー一覧に「シェイプ」として出て、選べた

- 原因: `useIframeSetup` の `initializeEditableElements` が紙面のすべての要素に `data-editable` と `data-element-id` を付け、
  `BR` を除外していなかった。`buildDomTreeInternal` は ID を持つ要素を全部レイヤーにし、`buildLayerLabel` は中身の無い要素を
  「シェイプ」と呼ぶ。キャンバスでも幅 0 の要素として選べた
- 直し方: `isNonEditableTag`(`utils/dom-utils.ts`、`BR` と `WBR`)
  - 印を付ける経路で飛ばす: iframe の初期化、部品の切り離し(`unlockPartDescendants`)、グループ解除、`makeChildrenEditable`
  - 前に付いていた印(`data-editable` / `data-element-id` / `data-inline`)は初期化で外す
  - レイヤーの木(`buildDomTreeInternal`)とクリックの解決(`getEditableElement`)でも、改行は親の文字要素として扱う

## 2. テキスト編集中に Enter を押すと、段落が分かれてスタイルが別になる

- 原因: contenteditable の既定の Enter。Chrome は `<p>` の中に `<div>` を差し込む(`<p>前半<div>後半</div></p>`)。
  HTML では p の中に div を置けないので、保存して読み直すと段落が閉じ、後半が段落のクラス(文字の大きさ・行間)を持たない div になる。
  Shift+Enter は同じ段落に `<br>` が入るだけで正しかった
- 直し方: `utils/text-line-break.ts` の `setupLineBreakOnEnter(doc)` を、`EditorCanvas` が紙面の document に張る(keydown の capture 1 本)。
  編集中の要素(`[contenteditable="true"]`)の中の Enter を `execCommand('insertLineBreak')` にする。Shift+Enter と同じブラウザの処理
  - 範囲選択の削除、行末の改行(次の行を作る 2 つ目の `<br>`)、編集中の ⌘Z 1 回での取り消し、`input` イベント(共同編集の送信)は
    ブラウザの処理がそのまま担う。自前で Range に `<br>` を入れると、ブラウザの取り消しの履歴に載らない
  - `insertLineBreak` を持たないブラウザでだけ自前で入れる(行末なら 2 つ目の `<br>` を置き、`input` イベントも投げる)
  - **日本語入力**: `isComposing` か `keyCode === 229` の keydown には触らない(最初に見る)。
    Safari は確定(`compositionend`)の後に Enter の keydown を送るので、確定から keyup までの Enter も見送る
  - 見送った Enter などが段落を分けようとしたとき(`beforeinput` の `insertParagraph`。Android の IME など)も改行に直す
  - ⌘ / Ctrl + Enter はショートカットに譲る
  - 対象はテキスト編集に入るすべての要素: p / 見出し / li / td / ボタン / span、テキストツールで置いた要素(`useDrawingMode`。同じ document なので同じ扱い)
  - 編集を抜けた後の ⌘Z は、今までどおりエディタの履歴(編集 1 回 = 1 段)

## 3. 右パネル「サイズ」の W / H の数値が切れて読めない

- 原因: `VariableAwareSizeInput` の固定値の箱が `w-14`(56px)決め打ちで、その中に数値とモード切替、外に単位(40px)を置いていた。
  さらにパネル共通の入力の見た目(`skin.css`: 高さ 32px・左右 8px の余白)が中の数値にも当たり、数値に使える幅が 26px しか無かった
- 直し方: 箱はモードによらず列の残りを使う(`flex-1 min-w-0`)。数値・単位・モード切替を箱の中に並べる。
  外側に `.group` を付けて共通の入力の見た目を中に当てず、箱は `.group.h-5`(数値のスクラブ入力と同じ見た目)にする。
  単位の `<select>` は既定の矢印を出さない。効いていなかった `:global()` の `<style>` を消した
- 数値の欄の幅(12px の文字。「1820.5」は 39px): パネル幅 280 / 303 / 480 で 54 / 65 / 154px

## 4. 最小・最大の幅と高さ(min-width / max-width / min-height / max-height)

- サイズ欄の右上の「+ 最小・最大」で、最小・最大の欄を出す(W の列に幅、H の列に高さ。ラベルは「最小」「最大」)。ラベルの列は W / H と同じ 24px にそろえ、どの行も箱の左端・右端が縦に並ぶ。値が入っている要素を選んだときは最初から出る。別の要素を選ぶと閉じる
- 欄は W / H と同じ `VariableAwareSizeInput`(`variant="limit"`)。px / % / vw・vh / em / rem と変数を扱う。空にすると指定を消す。モードの切替は出さない
- 適用は W / H と同じ `updateElementStyle`(複数選択なら全要素に同じ値、履歴 1 段)。Web ページ(構成ラフ)とスライドの両方
- 保存形は W / H と同じく、任意値のクラス(`min-w-[300px]`)とインラインの指定の両方。
  この 4 つは特殊値(`100%` → `min-w-full`)でもインラインを残す(`LAYOUT_CRITICAL_PROPERTIES`)。ページの Tailwind にクラスが無くても効き、欄に読み戻せる
- 読み戻し(`extractElementInfo` の `rawMinWidth` など): インライン → Tailwind の任意値・特殊値のクラス → 同じ系統の名前付きクラス(`max-w-6xl`)なら計算値。`none` / `auto` は指定なし
- `applyTailwindStyles` は空の値で、競合クラスに加えてインラインの指定も外すようにした(これまではクラスだけ消えていた)。呼び出し元を洗い出した結果:
  - `applyTailwindStyles` を呼ぶのは 4 か所(`updateElementStyle`、`pasteStyle`、`convertInlineStylesToTailwind`、文字の一部の書式)。
    `pasteStyle` と `convertInlineStylesToTailwind` は値が空でないものだけを集めて渡すので、空は届かない
  - `updateElementStyle` の直接の呼び出し 83 か所(EditorPropertyPanel 79・ScalePanel 3・`updateTextStyle` 1)、
    セクション部品の `onStyleChange` 50 か所(EditorSidebar などから 7 か所で渡す)、`updateTextStyle` 10 か所。空の値を直書きしている所は無い
  - 実行時に空が届くのは、最小・最大の欄を空にしたとき(意図どおり)と、`EditorSidebar` の部品の文字入力
    (FillSection の背景色・BorderSection の線色の 16 進、TypographySection の行高・字間)を空にしたときだけ。
    後者は「クラスだけ消えてインラインは残る」から「インラインも消える」に変わる。`EditorSidebar` はパッケージの中でも
    構成ラフ・デッキの殻でも描かれておらず、パッケージの入口(`exports` の `.` = `src/index.ts`)からも出していない
  - それ以外は空にならない(`VariableAwareUnitInput` は空欄を `0` + 単位で返す、W / H の箱は `auto`、選択肢・色は常に値を持つ)
- 選択枠のハンドルでのリサイズも、要素の min / max で止める(`computeResizeGeometry` の `limits`、開始時に計算値を読む。px で読めない軸は制約なし)。
  止まった軸を「効かないハンドル」と誤判定しない。左・上のハンドルでは、止まった分だけ対辺が動かない
- 最小の 0(`min-w-0` など。構成ラフのページに数百か所ある)だけの要素では欄を自動で開かない。0 は制約なしと見分けがつかないため。開けば 0 が入っている
- W / H と最小・最大の入力は、フォーカスが外れたら打ちかけの文字を捨てて要素の値を出す(打ちかけの文字を、次に選んだ要素の欄に残さない)

## 5. テキストの一部を選んでフォントサイズを変えると、選んだ所ではなく、それ以外の文字が大きくなる

**何が起きていたか**(playground と構成ラフの殻で再現): 実案件の EntryBand の段落は
`<p class="text-xs">お電話で…どうぞ<br><span class="text-sm font-bold">Tel. 03-6433-5163</span>（受付時間…）</p>`。

1. テキスト編集中に「Tel. 03-6433-5163」を選ぶと、書式ツールバーが出る(選択中の要素は段落の `<p>`)
2. 右パネルのサイズ欄を押すと、iframe からフォーカスが外れてテキスト編集は終わる(`contenteditable` が外れる)。
   **iframe の選択範囲はそのまま残っている**(フォーカスで選択が消えたのではない)
3. サイズ欄は `updateElementStyle({ fontSize })` で、選択中の要素 = 段落全体に `text-[32px]` を当てる。選択範囲は誰も読んでいない
4. 選んだ文字は自分の大きさ(`text-sm`)を持つ span なので、親の変更を受け継がず元の大きさのまま。
   段落の他の文字だけが大きくなる = 「選んだ所ではなく、それ以外が大きくなる」。
   選んだ所が素の文字なら段落全体が大きくなる(選んだ所も含めて全部)

書式ツールバー(`inline-format.ts`)が扱うのはマーカーと太字だけで、大きさ・色・字間の口は無かった。

**直し方**: `src/editor/utils/inline-text-style.ts`

- テキスト編集中に範囲を選んだら覚えておく(`selectionchange`)。パネルへフォーカスが移っても忘れない。
  紙面を押す・キャレットだけにする・要素の文字を全部選ぶ(編集に入った直後の全選択を含む)と忘れる
- 右パネルの文字の欄(書体・太さ・大きさ・字間・色・太字/斜体/下線)は `updateTextStyle` を通す。
  覚えている範囲がその要素の中にあれば範囲だけに当て、無ければ今までどおり `updateElementStyle`(要素全体)。
  行間・揃え・余白・塗りは範囲があっても要素全体
- 当て方: 範囲の文字をテキストノードごとに見て、行内要素の文字を丸ごと選んでいればその要素に当てる
  (「Tel. 03-6433-5163」なら既存の span の `text-sm` が `text-[32px]` に替わるだけで、包みは増えない)。
  一部だけならその部分を `<span>` で包む。包んだ中にある同じ指定は外し、何の指定も無くなった span はほどく。
  当てた後は範囲を当てた先の中身に張り直すので、「3」「2」と 1 桁ずつ打っても 2 回目からは同じ span に当たる
- 範囲に当てた値はクラスに加えてインラインにも書く。`font-bold` / `font-normal` / `italic` のような列挙の値はクラスだけだと、
  ページの CSS にそのクラスが無いと効かない。構成ラフの殻のコンパイル済み CSS には `italic` / `no-underline` / `font-extralight` が無い
  (実測)。提案書デッキの紙面は Tailwind Browser(`/vendor/tailwindcss-browser.js`)が付いたクラスをその場で作るので、クラスだけでも効く。
  (commit e4df3ab のメッセージの「デッキの CSS には font-normal が無く、変わらなかった」は確かめていない推測で、誤り。ここで直す)
- 範囲の中で値が揃っていなければ、サイズ・字間の欄は空で「混在」、ウェイトは「混在」、文字色は「文字色(混在)」と出す
- 保存は今までどおり DOM から(span のクラスとインラインの指定が残る)。履歴は `notifyIframeChange` で 1 段、
  共同編集は履歴の変化で相手に送られる
- PowerPoint 風のリボン(`PptChrome`)のフォントサイズ・太字・斜体・下線・字間・文字色と、`EditorSidebar` の `TypographySection` も同じ経路に通した。
  リボンの文字色はこれまで `applyTextColor` が今の選択を毎回新しい span で包んで選択を捨てていたため、色の入力が続けて change を出すと
  2 回目から段落全体に色が付き、span も重なっていた(実測)。範囲を覚えていれば大きさ・太字と同じ `apply` で同じ span に当てる

## 6. テキストを選んでからフレームの外をクリックすると、文字の選択が残ったままになる

**再現**(構成ラフのキャンバス表示・1 ページ表示、提案書デッキで確かめた):

| 操作 | 編集中に一部を選んでいたとき | 編集に入らずに文字を選んでいたとき |
|---|---|---|
| キャンバスの何も無い所をクリック | 選択も編集も残る(要素の選択だけ外れる) | 残る |
| キャンバスの別のページのフレームをクリック | 選択も編集も残る | (同じ経路) |
| 1 ページ表示・デッキの紙面の外の余白(iframe の中、ページの本文の外) | 消える(紙面の mousedown が編集を終える) | 残る |
| Esc | 消える | 残る |

**原因**:
- キャンバスの何も無い所の mousedown(`useCanvasMarquee`)は既定の動きを止めて要素の選択を外すだけで、紙面の iframe にフォーカスが残る。
  別のページのフレームの見本は `pointer-events: none` で、押すと親の側のフレームの枠が受ける。どちらも紙面の中の選択には誰も触らない
- 紙面の mousedown と Esc は、テキスト編集中の要素があるときだけ選択を消していた
- 文字の一部の書式(5 件目)が覚えた範囲も残るので、あとでパネルを操作すると古い範囲に当たりうる

**直し方**: `src/editor/utils/inline-text-style.ts`(クリックの処理 `useElementSelection` は組み替えない)

- 親の文書の pointerdown(capture): キャンバス(`[data-infinite-canvas]`)の中で、生きている紙面(`[data-editor-frame]`)の外を押したら、
  テキスト編集を終えて選択を消し、覚えた範囲も忘れる。要素の選択は既存の処理のまま
- 紙面の pointerdown: ページの本文(`#artboard`)の外の余白を押したら、編集に入っていない選択を消す(書式ツールバー・選択枠・ハンドル・共同編集やコメントの層は除く)
- Esc: 紙面では編集に入っていない選択を消す。親の文書では、フォーカスが入力欄やパネルに無いときだけ消す
- **消さない場面の見分け方**: 消すのは「キャンバスの中で紙面の外」と「紙面の中で本文の外の余白」だけ。右パネル・デッキのリボン・
  ポップアップ(ウェイトの選択肢・色見本など)はキャンバスの外にあるので対象にならない。書式ツールバーは紙面の中にあり、明示して除く

## 確かめたこと

Playwright(headless Chromium)で、`npm pack` した tgz を入れた構成ラフの殻・提案書デッキと、playground・ローカルの Hocuspocus で確かめた。

- `<br>`: `<br>` / `<wbr>` に印が付かない・前の印が外れる・レイヤーの段落の行に子が無い・改行の位置のクリックで段落が選ばれる
- Enter: 段落の途中(p は 1 つ・`<br>` +1・p の中の div 0)/ 編集中の ⌘Z 1 回 / 行末で Enter して打った文字が次の行に入る / 範囲選択して Enter /
  保存 HTML に `<br>` が入り div が無い / 読み直しても段落は 1 つでクラスが残る / 編集を抜けた後の ⌘Z 1 回で Enter の分だけ戻る /
  h2・li・td・button・span / テキストツールで置いた要素 / 共同編集に接続中(相手の紙面にも `<br>` が届き、段落は分かれない)
- 日本語入力: 合成の keydown(`isComposing: true` では入らない、対照の `false` では入る)と、
  CDP の `Input.imeSetComposition` で変換中の状態を作り、keyCode 229 の Enter → `Input.insertText` で確定(Chrome の順)、
  確定 → keyCode 229 の Enter(Safari の順)。どちらも `<br>` が入らず、確定の後の普通の Enter は改行になる
- サイズ欄: パネル幅 280 / 303 / 480 で W 1820・H 791 の固定値、「790.5」を打った状態で `scrollWidth <= clientWidth`・数値と単位とモード切替が重ならない。Fill / Hug と変数に紐づいた状態も列の中に収まる
- 最小・最大: 既定で隠れる → 4 つを入れると computed style に効く → W 800 / H 60 にしても 500 × 150 で止まる → ハンドルで広げても 500 で止まり、そのまま狭められる →
  保存 HTML に残る → 読み直すと最初から欄が出て値が戻る → 空にすると紙面と保存 HTML から消える → ⌘Z 1 回で戻る(Web ページとスライドの両方)。`min-w-0` だけの要素では自動で開かない
- テキストの一部(5 件目): EntryBand と同じ形の段落で、編集中に「Tel. 03-6433-5163」を選ぶ → パネルに「選んだ文字だけ」とその文字の大きさが出る →
  サイズ 32 で電話番号だけが 32px、段落の他の文字は元のまま(span は増えず `text-sm` が外れる・段落のクラスは変わらない)→ 保存 HTML の span に残り段落のタグには付かない →
  読み直しても同じ大きさ → 素の文字の一部(お電話)だけが 20px になり span が 1 つ増える → ⌘Z 1 回で span ごと戻る →
  大きさの違う文字をまたぐと「混在」→ 太さ・字間は選んだ所だけ、行間は段落全体 → 範囲を選ばない・文字を全部選んだときは要素全体(span を作らない)→
  共同編集に接続中でも相手の紙面に一部の文字だけの大きさが届く。
  構成ラフの殻で段落の一部だけを斜体にできる(殻の CSS に `italic` が無く、インラインの指定で効く)。提案書デッキでも太字の段落の一部を Regular(400)にできる
- リボン(提案書デッキの PowerPoint 風の画面): 一部を選んでフォントサイズ 48 → 選んだ所だけ変わり ⌘Z 1 回で戻る → 太字も同じ →
  文字色を選び直さずに 2 回変えても選んだ所だけ・⌘Z 1 回で直前の色 → 大きさ・太字・色が同じ span 1 つに載る → 保存 HTML に残り、読み直しても同じ
- サイズ欄のそろい: パネル幅 280 / 303 / 480 で、最小・最大の 4 つに 1820 / 790.5 を入れた状態で、W・最小・最大の箱の左端・右端が同じ位置、H の列も同じ。ラベルと数値の `scrollWidth <= clientWidth`
- フレームの外で選択を消す(6 件目): キャンバスの何も無い所(編集中・編集外)、別のページのフレーム、1 ページ表示とデッキの紙面の外の余白(編集中・編集外)、Esc で選択が消える。
  右パネルのフォントサイズ・書式ツールバーの太字・デッキのリボンの太字は選んだ所だけに効き、選択を消さない。同じフレームの別の段落のクリックは今までどおり。
  共同編集に接続中も紙面の外の余白で消える。直す前の版では、消す 6 件が落ち、残す側の確認は通ることを確かめた
- わざと壊した版で落ちること: `<br>` の除外を外す → 3 件 FAIL、日本語入力の判定を外す → 4 件 FAIL

## 分かっている穴

- 日本語入力は headless Chromium で CDP と合成イベントを使って確かめた。実機の IME(macOS / Windows)、Safari、Firefox では確かめていない。
  `insertLineBreak` を持たないブラウザ向けの自前の差し込みは、ブラウザの取り消しの履歴に載らない(編集を抜けた後のエディタの履歴では戻る)
- W / H の数値は、今までどおり実測値を整数に丸めて出す(790.5px の要素は 791)。欄の幅は「1820.5」まで入る
- 最小・最大の欄が読むのは要素自身の指定(インライン・Tailwind のクラス)だけ。ページの CSS のセレクタで付いた min / max は出ない
- `beforeinput` の `insertParagraph` を改行に直す経路(keydown を見送ったのに段落を分けようとしたとき)は、どの検証でも通っていない
- 複数選択での最小・最大は W / H と同じ `updateElementStyle`(選択中の全要素)に乗せただけで、検証していない
- テキストの一部(5 件目)で確かめたのは、右パネルのサイズ・ウェイト・字間・行間、共同編集のサイズ、リボンのフォントサイズ・太字・文字色。
  右パネルの書体・文字色・太字/斜体/下線のボタン、リボンの斜体・下線・字間、`TypographySection` は同じ経路に通したが、画面では確かめていない。
  `TypographySection` とリボンは「混在」を出さない(リボンは揃っているときだけ範囲の値を出す)
- リボンのサイズ欄は 6〜400 以外の途中の値を受け付けずに欄を戻すので、「48」を 1 文字ずつ打つと「4」で戻って入らない。
  候補から選ぶ・拡大/縮小のボタンは効く。今回の変更の前からの動きで、直していない(検証は値をまとめて入れる)
- 編集を抜けたときの履歴の記録は `postMessage` で遅れて届く。自動操作で欄を押した直後(0ms)に打つと、
  遅れて届いた編集前のスナップショットが履歴の今の段を上書きし、⌘Z が効かないように見えることを実測した(紙面の変更は残る)。
  人が押してから打つまでの間には届くので、検証は欄を押して 150ms 置いてから打つ。今回の変更の前からある経路で、直していない。
  リボンのボタンも同じで、押してから離すまで 0ms の自動操作のクリックでは太字の ⌘Z が効かないように見えた(120ms 置くと戻る)
- 1 桁ずつ間を空けて打つと(400ms 超)、最初の 1 桁で span を作った段と次の段が分かれ、⌘Z が 2 回要る(W / H と同じまとめ方)
- 範囲の中の空白だけのテキストノードは包まない。ブロック要素をまたぐ範囲(テキストノードごとに包む)と、
  覚えている範囲の段落を共同編集の相手が書き換えた場合(範囲が消えれば要素全体の扱いに戻る)は確かめていない
- 範囲を選ばずに要素全体を変えたとき、中の span が持つ自分の大きさは今までどおり残る(要素全体の経路は変えていない)
- フレームの外で選択を消すのは、キャンバスの中と紙面の中の余白だけ。ページのレール・レイヤー一覧・上の帯を押しても紙面の選択は残る
  (右パネル・リボン・ポップアップの操作で選択を消さないことを優先して、消す場所を広げていない)。
  編集に入らない文字の選択は、検証では Range で作った(実機では文字をなぞる・ダブルクリックでできる)
- ハンドルで止めるのは計算値が px の軸だけ。% などの min / max はハンドルでは止めない(紙面では CSS が効く)。複数選択の枠のリサイズは要素ごとの min / max を見ない
