...
- 紙面のリンクを ⌘ + クリック(Windows は Ctrl + クリック)すると、リンク先へ移る。同じサイトのページは `contentList[].href` と照らし合わせてそのページを編集中にし(表記の揺れ `/company/`・`index.html`・相対パス・`?`・`#` も同じページ)、`#id` はその場所へ紙面を寄せ、別のサイトは新しいタブで開く。仕組みと既存の ⌘ クリックとの両立は `docs/link-nav-2026-09.md`
