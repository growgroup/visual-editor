...
- `contentList[].thumbnail` にページ全体を写した画像(PNG / JPEG、幅 480px 以上、縦横比はページと同じ)の URL を渡すと、大きく縮めた表示(倍率 0.5 未満)で紙面が iframe ではなくその画像になり、縮小が軽くなる。縮小している最中も、0.4 を割った時点で(画像が読めていれば)切り替わる。実測で縮小の p95 が 26.5ms → 18.4ms、50ms を超えるフレームが 0 回。内容が変わったら URL も変える(`&v=…`)。渡さないページ・画像が読めなかったページは今までどおり iframe(0.4.0)
...
| したいこと | 操作 |
|---|---|
| この帯の中だけ自由に置く | 器を右クリック → **子をすべて絶対配置にする** |
...
設計と判断の経緯、確かめていないことは [docs/free-layout-2026-09.md](./docs/free-layout-2026-09.md) にあります。
