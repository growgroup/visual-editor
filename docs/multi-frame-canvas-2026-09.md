...
- 判定は**操作が止まってから**しか動かさない(縮小の途中で 26 枚を差し替えない)。行き来でちらつかないよう、画像 → iframe は 0.5 以上、iframe → 画像は 0.4 未満(ヒステリシス)
...
実測(playground の 26 ページ、同じ条件): 紙面が iframe のまま p95 33.4ms → 画像に落ちている状態 **p95 17.4ms**。
